...
Вход закрыт страницей логина внутри самого приложения.
...
Скрипт спросит логин и пароль, посчитает bcrypt-хеш и ключ подписи сессии
и запишет всё в `.env`. Считает локально, через `htpasswd` — в сеть не ходит,
поэтому лимиты Docker Hub на этом шаге не мешают.

Этими данными пользователь входит на **страницу входа самого приложения**
(своя форма с логотипом, а не системное окно браузера).
...
```bash
docker compose config | grep APP_PASSWORD_HASH
```
...
> `TRADEMAP_APP_USER` и `TRADEMAP_APP_PASSWORD_HASH` — вход в само приложение.
...
Откройте `https://trademap.example.com` — появится страница входа с логином
и паролем из `./setup-auth.sh`. После входа значок в правом верхнем углу
должен показать «TradeMap: подключено» — значит, приложение авторизовалось
в TradeMap само.

Обратите внимание: это два разных входа. Форма на странице пускает **в
приложение**; бейдж справа сверху показывает состояние подключения **к
trademap.org**, которое приложение устанавливает само по `.env`.
...
**У приложения нет разграничения прав.** Вход один на всех: любой, кто знает
пароль, работает от вашей учётной записи TradeMap. Сессия живёт две недели
(`TRADEMAP_SESSION_MAX_AGE` в секундах), кнопка «Выйти» её сбрасывает. Для более узкого доступа лучше не открывать домен наружу, а ходить
через SSH-туннель:
